...
cdef class quadMeshProcessing :
    ...
    def __init__(self, verts, faces, 
                    verts_label_map=None, faces_label_map=None):
        ...
        print('Faces.shape =',faces.shape,'Verts.shape =',verts.shape)
    ...
    cdef correct_winding_faces(self):
        ...
        for i in range(n_quads):
            ...
            new_indexing = self.correct_winding_1face(quad_mv)
    ...
    cdef int checkIfFaceCooplanar(self, double[:,:] quad, double thresh = 1e-10):
    ...
    cdef Py_ssize_t[:] correct_winding_1face(self, double[:,:] quad_face):
        """Checks the winding for one face and returns the new
        order of the indices 
        """
        cdef :
            SymmetricMatrix distance_matrix 
        ...
        distance_matrix = intra_face_distance_matrix(quad_face)
    ...
    cdef getQuads(self):
        cdef size_t[:] fs = np.array(self.faces_mv.shape, dtype = np.uint)
...
cdef vector[vector[vector[double]]] cgetQuads(int[:,:] faces_mv,
               size_t[:] fs, double[:,:] verts_mv, int[:] verts_label_map_mv):
    cdef: 
        size_t idx, i, j
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef SymmetricMatrix intra_face_distance_matrix(double[:,:] quad_face):
    cdef :
        SymmetricMatrix m = SymmetricMatrix(size=4)
        Py_ssize_t i, j 
        double[:] pt1, pt2
    for i in range(4):
        for j in range(i,4):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[double] cross_product_vect(vector[double] vect1, vector[double] vect2):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[double] quad_centroid(double[:,:] quad):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[vector[double]] quadCenterEdgeVectors(double[:,:] quad, vector[double] centroid):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[double] quadNormal(double[:,:] quad):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[double] vector2unit(vector[double] vect):
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef double norm_vector(vector[double] vect):
    """norm of n dimensional vector
    """
...
@cython.wraparound(False)
@cython.boundscheck(False)
cdef vector[double] sum_vectors(vector[double] vect1, vector[double] vect2):
